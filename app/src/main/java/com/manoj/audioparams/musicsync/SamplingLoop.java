 ...
 *
 *    musicsync code has been taken from-https://github.com/bewantbe/audio-analyzer-for-android
 */
...
public class SamplingLoop extends Thread {
    ...
    private Service musicSyncService;
    ...
    public SamplingLoop(AnalyzerParameters _analyzerParam) {
        analyzerParam = _analyzerParam;
        //isPaused1 = ((SelectorText) activity1.findViewById(R.id.run)).getValue().equals("stop");
        // Signal sources for testing
        double fq0 = Double.parseDouble(musicSyncService.getString(R.string.test_signal_1_freq1));
        double amp0 = Math.pow(10, 1 / 20.0 * Double.parseDouble(musicSyncService.getString(R.string.test_signal_1_db1)));
        double fq1 = Double.parseDouble(musicSyncService.getString(R.string.test_signal_2_freq1));
        double amp1 = Math.pow(10, 1 / 20.0 * Double.parseDouble(musicSyncService.getString(R.string.test_signal_2_db1)));
        double fq2 = Double.parseDouble(musicSyncService.getString(R.string.test_signal_2_freq2));
        double amp2 = Math.pow(10, 1 / 20.0 * Double.parseDouble(musicSyncService.getString(R.string.test_signal_2_db2)));
...
//      FPSCounter fpsCounter = new FPSCounter("SamplingLoop::run()");
        ...
        boolean bSaveWavLoop = false;  // change of bSaveWav during loop will only affect next enter.
        if (bSaveWavLoop) {
            wavWriter.start();
...
//          fpsCounter.inc();
                ...
                stft.calculatePeak();
               // musicSyncService.maxAmpFreq = stft.maxAmpFreq;
               // musicSyncService.maxAmpDB = stft.maxAmpDB;
    ...
    public Pair<Double, Double> getValues(){
        return new Pair(stft.maxAmpFreq, stft.maxAmpDB);
    }
